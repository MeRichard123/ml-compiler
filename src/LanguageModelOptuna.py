...
def objective(trial, model, device, text, vocab):
    # Create model instance
    lm = LanguageModelOptuna(model, device)
    ...
    try:
        avg_loss = lm.train_loop(n_iters=n_iters, learning_rate=learning_rate)
        return avg_loss
    except Exception as e:
        LOGGER(f"Trial failed with error: {str(e)}")
        raise optuna.TrialPruned() 
    
def tune_hyperparameters(model, device, text, vocab, n_trials=50):
    study = optuna.create_study(direction='minimize')
    study.optimize(lambda trial: objective(trial, model, device, text, vocab), 
                n_trials=n_trials)
...
best_params = tune_hyperparameters(model, device, text, vocab, n_trials=50)
